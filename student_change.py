...
if Submit is not None:
    q = """SELECT * FROM batch_details WHERE email='%s' AND register_no='%s'""" % (Email, Regno)
    cur.execute(q)
    res = cur.fetchall()
    con.commit()
    ...
    if NewPassword == ConfirmPassword:
        q = """UPDATE student_login SET password='%s' WHERE username='%s'""" % (NewPassword, Regno)
        cur.execute(q)
        con.commit()
        print("""
        <script>
