...
<body>
    ...
    <h1 style=text-align:center;>Semester 4</h1>
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="summary">
                ...
                <input type="number" step="0.01" id="gpa" name="gpa" value="{result[54]}" required>
                 ...
                 <h3 style="text-align:center">Scholarship Details : Year 2 (If Any)</h3>

                <label for="scholarship_name">Scholarship Name:</label>
                <input type="text"  id="scholarship_name" name="scholarship_name" value="{sch[6]}">

                <label for="scholarship_amount">Scholarship Amount:</label>
                <input type="text"  id="scholarship_amount" name="scholarship_amount" value="{sch[7]}" >
...
ScholarshipName = form.getvalue("scholarship_name")
ScholarshipAmount = form.getvalue("scholarship_amount")
...
if Submit != None:
    ...
    if (res == None):
        ...
        q = """SELECT id FROM scholarship_details WHERE register_no='%s'""" % (Regno)
        cur.execute(q)
        rec = cur.fetchone()

        if rec != None:

            q = """UPDATE scholarship_details SET name2='%s' AND amount2='%s' WHERE register_no='%s'""" % (
                ScholarshipName, ScholarshipAmount, Regno)
            cur.execute(q)
            con.commit()

        else:
            q = """INSERT INTO scholarship_details(register_no,name2,amount2) VALUES ('%s','%s','%s')""" % (
                Regno, ScholarshipName, ScholarshipAmount)
            cur.execute(q)
            con.commit()
        ...
        print("""
            <script>
             alert("Semester 4 Form Filled Successfully !");
    ...
    if (res!= None):
        ...
        print("""
                    <script>
                     alert("Semester 4 Form Updates Successfully !");
                     location.href='student_main.py?id=%s'
                    </script>    
                    """ % Regno)
